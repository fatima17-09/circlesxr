...
AFRAME.registerComponent('spinning-effect', {
    ...
    init: function() {
      ...
      Context_AF.rocket.setAttribute('animation', {property:'rotation.z', to:360, loop:true, dur:Context_AF.data.duration, easing:'linear', enabled:false});
      
      //listen on click
      Context_AF.el.addEventListener('click', function() {
        if (Context_AF.isSpinning === true) {
          console.log('stop spinning');
          Context_AF.rocket.setAttribute('animation', {enabled:false});
          Context_AF.isSpinning = false;
        }
        else {
          console.log('spinning');
          Context_AF.rocket.setAttribute('animation', {enabled:true});
